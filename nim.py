...
class NimAI():
    ...
    def choose_action(self, state, epsilon=True):
        ...
        # If epsilon is False
        if not epsilon :
            action = self.find_best_action_available(state)
    ...
    def find_best_action_available (self , state):
        available_actions = Nim.available_actions(state)
        dict_actions = {}
        for action in available_actions:
            if action not in dict_actions.keys():
                q_value = self.get_q_value(state,action)
                dict_actions[action] = q_value
            
        #  descending 
        sorted_list =  sorted(dict_actions, key=lambda key: dict_actions[key],reverse=True)    
